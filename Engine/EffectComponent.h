...
namespace Engine
{
	...
	{
	public:
		EffectComponent(GameObject* gameObject) : Component(gameObject) {}
		...
		void AddHitEffect(float duration = 0.2f) {
			...
			if (!spriteRenderer || !transform) return;
		...
		void Update(float deltaTime) override {
			for (auto& effect : effects) {
				...
				if (effect.updateTransform) effect.updateTransform(progress);
				if (effect.updateColor) effect.updateColor(progress);
			}
